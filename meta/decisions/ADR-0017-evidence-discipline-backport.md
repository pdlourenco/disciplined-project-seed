...
anecdote, which changes its evidence standing.
...
2. **The tells/instances split is the load-bearing convention.** Numbered
   ...
   without it.
3. **#55 lands as tell 7** (a summarizing intermediary treated as a primary
   source) rather than as a standalone convention. It is admitted without the
...
- **Tell 7 as a clause on tell 2** ("unchecked tool state"): lighter, but
  tell 2 is about tool *state* left unchecked, while the fetch-tool failure is
  the tool's *permanent mechanism* — and buried as a git-flavoured example it
  would not be recalled at research time, which is author-side, before review.
