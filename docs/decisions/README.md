...
- **Parallel tracks: rebase before finalizing the number.** When two tracks (e.g. parallel agent sessions) both append to this sequence, a number assigned off a shared merge-base collides at the *second* merge. Reserve a per-track band **and rebase onto `main` before finalizing the number** — the rebase surfaces an in-flight neighbour the merge-base hid, which is otherwise discovered only as a filename / number collision (or a red contract test) at the second merge. The same rule applies to any identifier drawn from a sequence that parallel tracks both append to — SPEC version strings and set-for-set catalogues among them, not just ADR numbers.
- **Scan in-flight work too.** The rebase only surfaces *merged* neighbours; before claiming a number, also scan open PRs — `gh pr list --state open --limit 200 --json number,title,files --jq '.[] | select([.files[].path] | any(contains("docs/decisions/ADR-")))'` — as well as merged history. (`--limit 200` defeats `gh pr list`'s silent 30-PR default; the path is scoped to `docs/decisions/` so it doesn't match the seed's separate `meta/decisions/` namespace — drop that scope if you stripped `meta/`.) Filter on the changed-file **path**, not `--search 'ADR- in:files'`: `in:files` is not a supported issue/PR search qualifier (GitHub doesn't index PR changed files), so it is silently ignored and the search degrades to a title/body text match — missing exactly the in-flight ADR that doesn't name itself in its title or body, which is the case the scan exists for. The `--json files` field does read the PR's changed files — but note it returns at most the first ~100 changed files per PR and does not paginate, so a mega-PR whose ADR sits past that cap is still silently missed. Projects with frequent parallel tracks, or where PRs that large are plausible, can promote this to a small CI uniqueness check (ADR-number / SPEC-version uniqueness against open PRs), which can page the files API properly.
- **Banding for timestamped sequences.** For identifiers that are timestamps rather than serials (e.g. migration filenames), the per-track band takes the form of distinct leading-decade blocks — band per active track or phase, re-banding to a fresh decade when a shared one exhausts.
...
<!-- Your first entries here. The seed's own ADRs about how the seed
     was built live in `meta/decisions/` (separate namespace, separate
     numbering) — they're not listed here so this index stays your
     project's index. -->
