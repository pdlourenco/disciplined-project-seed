...
- **Parallel tracks: rebase before finalizing the number.** When two tracks (e.g. parallel agent sessions) both append to this sequence, a number assigned off a shared merge-base collides at the *second* merge. Reserve a per-track band **and rebase onto `main` before finalizing the number** — the rebase surfaces an in-flight neighbour the merge-base hid, which is otherwise discovered only as a filename / number collision (or a red contract test) at the second merge. The same rule applies to any identifier drawn from a sequence that parallel tracks both append to — SPEC version strings and set-for-set catalogues among them, not just ADR numbers (one adopter hit a live merge-blocker on an ADR number *and* a SPEC version in the same PR, with the rebase rule on paper).
- **Scan in-flight work too.** The rebase only surfaces *merged* neighbours; before claiming a number, also scan open PRs — `gh pr list --search 'ADR- in:files'` — as well as merged history. Projects with frequent parallel tracks can optionally promote this to a small CI uniqueness check (ADR-number / SPEC-version uniqueness against open PRs).
- **Banding example — timestamped sequences.** For identifiers that are timestamps rather than serials (e.g. migration filenames), the per-track band takes the form of distinct leading-decade blocks: one adopter banded migration timestamps per active phase after three recorded collisions, re-banding a phase to a fresh decade when a shared one exhausted.
