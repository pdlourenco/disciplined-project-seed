...
- **The audit needs a skip list for template files.** Files that exist specifically as fill-in templates for adopters (PHASE-TEMPLATE, ADR-TEMPLATE, SPEC, DESIGN, ROADMAP, REVIEW_CONTEXT, STRUCTURE) would false-positive otherwise. The skip list is duplicated across two places — the `find` invocation in the `placeholder-audit` job and the `ignores` array in `.markdownlint-cli2.jsonc` — and must be kept in sync by hand. Promote to a shared config when the seed has more than two consumers of the list (e.g. when link-check or a new gate also needs it).
