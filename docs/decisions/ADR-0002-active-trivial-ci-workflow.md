...
- **The audit needs a skip list for template files.** Files that exist specifically as fill-in templates for adopters (PHASE-TEMPLATE, ADR-TEMPLATE, SPEC, DESIGN, ROADMAP, REVIEW_CONTEXT, STRUCTURE) would false-positive otherwise. The skip list lives in the workflow's `find` invocation; adopters edit it as their files mature.
...
- **`.markdownlint.json` at the root** disables MD013 (line length — the seed has long prose lines) and MD033 (inline HTML — the seed uses `<!-- ... -->` everywhere). Other rules at defaults.
