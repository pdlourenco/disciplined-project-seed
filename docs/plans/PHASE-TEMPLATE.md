...
- **Randomized-exploration campaign run** — if the project has adopted `CONTRIBUTING.md` §"Randomized-exploration testing (PBT / Monte-Carlo)", run the unbounded campaign at phase completion and triage failures (promote each failing case to a committed deterministic fixture). This is the convention's mandatory floor; delete this bullet only if the convention isn't adopted.
