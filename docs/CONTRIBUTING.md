...
**Drift detection.** [`.github/workflows/check-branch-protection.yml`](../.github/workflows/check-branch-protection.yml) runs weekly (Sundays 10:00 UTC) with a **two-layer** model. Layer 1 (always runs) is a *presence check* — `GET /repos/.../branches/main` returns a `protected` boolean readable with the default `GITHUB_TOKEN`'s `contents: read` scope; this catches "protection removed entirely". Layer 2 (opt-in) is *field-level drift* — `GET /repos/.../branches/main/protection` requires `administration: read` scope, which the default token does not have; adopters who want detailed drift provide `BRANCH_PROTECTION_READ_TOKEN` (PAT or App token) as a repo secret. Without it, layer 1 still catches presence drift. The error handler distinguishes HTTP 403 (denied — token lacks scope) from 404 (missing — no protection set), so denied-as-absent is never mis-reported. If you edited the YAML and forgot to re-apply, the workflow will tell you within a week; manual edits to `Settings → Branches` get caught too.
