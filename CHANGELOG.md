...
- `.markdownlint-cli2.jsonc` — single config file (markdownlint-cli2's native format) containing the rule config (MD013 / MD024 / MD033 / MD041 disabled), the glob (`**/*.md`), and the ignore list matching the placeholder-audit and link-check jobs. MD024 (no-duplicate-heading) is fully off — the seed's templates use comment-placeholder headings (`#### <!-- Example: ... -->`) that all strip to empty text and false-positive. The reviewer subagent catches real heading-hygiene drift. Replaces the original `.markdownlint.json` + `.markdownlintignore` pair, which markdownlint-cli2 silently ignored.
