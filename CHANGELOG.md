...
- `.markdownlint-cli2.jsonc` — single config file (markdownlint-cli2's native format) containing the rule config (MD013 / MD033 / MD041 disabled; MD024 `siblings_only: true`), the glob (`**/*.md`), and the ignore list matching the placeholder-audit and link-check jobs. Replaces the original `.markdownlint.json` + `.markdownlintignore` pair, which markdownlint-cli2 silently ignored.
