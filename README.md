...
1. **Start from the seed.** Three clean-start paths:
   - Click *Use this template* on the [GitHub repo](https://github.com/pdlourenco/disciplined-project-seed) for a clean history.
   - Download the **template archive** attached to the [latest release](https://github.com/pdlourenco/disciplined-project-seed/releases/latest) (`*-template.zip` / `*-template.tar.gz`) — it is the released tree with `meta/` **already stripped**, so step 2 is done for you and you land on a pinned version. (The release page also carries that version's CHANGELOG section, which becomes your `DISCIPLINE_ADOPTION.md` pin.)
   - Clone-and-strip-history manually.
2. **Decide what to do with `meta/`.** The seed ships its own evolution history (ADRs about how the seed was built, plus a CHANGELOG) under [`meta/`](meta/). *(If you started from the template archive above, `meta/` is already gone — skip to keeping-vs-stripping only if you want it back for reference.)* Two paths:
...
Four adoption profiles. The shared scaling rule: the seed scales down by **dropping files, not by diluting rules** — don't weaken ADRs, pre-push review, contracts-with-teeth, or recommend-don't-decide.
...
- **Governance retrofit** — for a project that already has its own contract core: adopt only the governance layer, adapted to the existing layout — adapt to the project's layout, don't impose the seed's. Mechanics: **one artifact per stacked PR**, strictly ordered review; **forward-reference bookkeeping** (an early PR may point at conventions still to land — via issue links each later PR closes); a **retroactive ADR** anchoring a pre-existing principle the project already operated under, so the rationale is recoverable without pretending the decision is new.

Two further patterns, whatever the profile:

- **Pre-existing design material**: keep it verbatim in a frozen directory (e.g. `notes/`) marked "not authoritative", migrate the load-bearing parts into the disciplined `docs/` tree with a mapping table, and keep the old TODO scratchpad with resolved items struck through as an audit trail.
- **Vocabulary reconciliation**: before importing the seed's names (tiers, labels, section titles), diff them against local usage. Where a name already carries a different local meaning, don't adopt it as-is — a colliding name is actively misleading. Reconcile first; renaming after adoption is churn.
...
- **The update feed:** each tag is published as a [GitHub release](https://github.com/pdlourenco/disciplined-project-seed/releases) (release-as-code, [ADR-0015](meta/decisions/ADR-0015-release-as-code.md)) — **watch → releases** to be notified of new seed versions. Every release carries its CHANGELOG section as the body, a `vA...vB` compare link for the exact diff between two pins, and the pre-stripped template archive. The release is gated: the tag only publishes if its CHANGELOG section is present and dated and `[Unreleased]` is empty, so a pin always lands on a real, correctly-cut version.
